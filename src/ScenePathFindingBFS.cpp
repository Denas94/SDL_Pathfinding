...
void ScenePathFindingBFS::drawMaze()
{
	if (draw_grid)
	{
...
void ScenePathFindingBFS::drawCoin()
{
...
void ScenePathFindingBFS::initMaze()
{
...
void ScenePathFindingBFS::initGraph() {
...
bool ScenePathFindingBFS::loadTextures(char* filename_bg, char* filename_coin)
{
...
Vector2D ScenePathFindingBFS::cell2pix(Vector2D cell)
{
	int offset = CELL_SIZE / 2;
	return Vector2D(cell.x*CELL_SIZE + offset, cell.y*CELL_SIZE + offset);
}

Vector2D ScenePathFindingBFS::pix2cell(Vector2D pix)
{
	return Vector2D((float)((int)pix.x/CELL_SIZE), (float)((int)pix.y / CELL_SIZE));
}

bool ScenePathFindingBFS::isValidCell(Vector2D cell)
{
...
void ScenePathFindingBFS::BFS(Node objectiu, Node start){
	...
	//Per quan arrivi a l'objectiu netegi
	path.points.clear();
	
	while (!frontera.empty()) {
		Node nodeActual = frontera.front();
		...
		if (!visited[currentX][currentY]) {
			...
			for (int i = 0; i < neighbors.size(); i++) {
				if (!From[neighbors[i].position.x][neighbors[i].position.y].visited) {
